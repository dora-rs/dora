...
fn native_runtime_command(node_id: &NodeId) -> eyre::Result<Command> {
    ...
    if file_name.ends_with("python") || file_name.ends_with("python3") {
        // Use the current executable to spawn runtime. That lands on
        // `dora-runtime-python`, which hosts shared-library operators by
        // delegating to `dora-runtime-shared-lib` — see that crate's module
        // docs. Keep the two in step: dropping the delegation breaks native
        // operators under an embedded-Python daemon.
        let python =
