...
use adora_node_api::{Event, Parameter, merged::MergedEvent};
use adora_operator_api_python::PyEvent;
use adora_operator_api_types::AdoraStatus;
...
pub fn run(
    ...
    let python_runner = move || {
        ...
        let reason = loop {
                ...
                match status_enum {
                    ...
                    Err(err) => {
                        if reload {
                            ...
                            warn!("{err}");
                            Ok(AdoraStatus::Continue as i32)
                        } else {
                            Err(err)
                        }
                    }
            ...
            match status {
                s if s == AdoraStatus::Continue as i32 => {} // ok
                s if s == AdoraStatus::Stop as i32 => break StopReason::ExplicitStop,
                s if s == AdoraStatus::StopAll as i32 => break StopReason::ExplicitStopAll,
...
mod callback_impl {
    ...
    /// - the third argument is adora metadata if you want to link the tracing from one input into an output.
    /// `e.g.:  send_output("bbox", pa.array([100], type=pa.uint8()), adora_event["metadata"])`
    #[pymethods]
    impl SendOutputCallback {
        ...
        fn __call__(
            ...
            let otel = if let Some(adora_node_api::Parameter::String(otel)) =
