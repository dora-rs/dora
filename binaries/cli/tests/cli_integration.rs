use clap::Parser;
use dora_cli::cli::{Cli, Command, OutputFormat, UiMode};

#[test]
fn test_cli_parsing() {
    // Test basic command parsing
    let cli = Cli::try_parse_from(["dora", "ps"]).unwrap();
    assert!(matches!(cli.command, Some(Command::Ps(_))));
}
...
fn test_three_tier_structure() {
    ...
    assert!(matches!(cli.command, Some(Command::Debug(_))));

    let cli = Cli::try_parse_from(["dora", "analyze"]).unwrap();
    assert!(matches!(cli.command, Some(Command::Analyze(_))));
    ...
    assert!(matches!(cli.command, Some(Command::Dashboard(_))));
}
