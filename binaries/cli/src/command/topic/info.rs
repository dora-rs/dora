...
fn info(
    ...
    rt.block_on(async move {
        ...
        let duration_duration = Duration::from_secs(duration);

        // Collect messages for the specified duration
        while start_time.elapsed() < duration_duration {
            if let Ok(sample) = tokio::time::timeout(
                duration_duration - start_time.elapsed(),
    ...
    let hz = stats.calculate_hz(Duration::from_secs(duration));
    ...
    if duration > 0 {
        let bandwidth_mbps = (total_bytes as f64 * 8.0) / (duration as f64 * 1_000_000.0);
