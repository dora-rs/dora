...
fn import_ros_messages(args: Import) -> eyre::Result<()> {
    ...
    for message_type in &args.message_types {
        ...
        if parts.len() != 2 {
            errors.push(format!(
                "Invalid message type format: {}. Expected: package/MessageName",
                message_type
            ));
            continue;
        }
    ...
    println!(
        "\n✓ Successfully imported {} ROS message types",
        imported_count
    );
    ...
    let msg_file = if let Some(custom_path) = msg_path {
        ...
        dora_ros_compat::message_parser::find_msg_file(&default_workspace, package, message)
            .or_else(|_| {
                // Try ROS 2 location
                let ros2_distro =
                    std::env::var("ROS_DISTRO").unwrap_or_else(|_| "humble".to_string());
...
fn generate_rust_binding(
    msg_def: &dora_ros_compat::message_parser::MessageDefinition,
) -> eyre::Result<String> {
    let mut code = String::new();

    code.push_str(&format!(
        "//! Auto-generated Rust binding for {}/{}\n",
...
fn generate_python_stub(
    ...
    code.push_str(&format!(
        "\"\"\"Auto-generated Python binding for {}/{}\"\"\"\n",
        msg_def.package, msg_def.name
    ));
    code.push_str("# Generated by dora ros import\n\n");
