...
impl ExecutionContext {
    ...
    pub fn detect_basic() -> Self {
        ...
        Self {
            ...
            is_tty: Self::detect_tty(),
            is_piped: Self::detect_piped(),
            is_scripted: Self::is_ci_environment(),
            terminal_size: Self::detect_terminal_size(),
    ...
    fn detect_scripted() -> bool {
        ...
        if let Ok(parent) = Self::get_parent_process() {
            matches!(parent.as_str(), "bash" | "sh" | "zsh" | "fish" | "python" | "node")
        } else {
            false
        }
    }
...
impl ExecutionEnvironment {
    ...
    pub fn detect() -> Self {
        let env_vars: HashMap<String, String> = std::env::vars()
            .filter(|(key, _)| {
                // Only collect relevant environment variables
                key.starts_with("CI") || 
                key.starts_with("GITHUB") ||
                key.starts_with("GITLAB") ||
                key.starts_with("JENKINS") ||
                key.starts_with("BUILDKITE") ||
                key.starts_with("CIRCLE") ||
                key.starts_with("TRAVIS") ||
                key.starts_with("APPVEYOR") ||
                key.starts_with("AZURE") ||
                key.starts_with("TEAMCITY") ||
                key.starts_with("DORA") ||
                matches!(key.as_str(), "TERM" | "SHELL" | "COLORTERM" | "TERM_PROGRAM")
            })
            .collect();
    ...
    pub fn detect_basic() -> Self {
        ...
        Self {
            ci_environment: if is_ci { Some(CiEnvironment::Other("CI".to_string())) } else { None },
            shell_type,
...
impl TerminalCapabilities {
    ...
    pub fn detect() -> Self {
        ...
        let tui_capable = interactive && 
                         width.unwrap_or(0) >= 80 && 
                         height.unwrap_or(0) >= 24 &&
                         supports_color;
