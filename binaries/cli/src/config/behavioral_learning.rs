...
impl BehavioralLearningEngine {
    ...
    /// Analyze patterns and update preferences
    async fn analyze_and_update_patterns(&self, prefs: &mut UserPreferences) -> eyre::Result<()> {
        // Get recent choices for analysis
        let recent_choices: Vec<_> = prefs
            .behavior
            .interface_choices
            .iter()
            .rev()
        ...
        let command_patterns = self
            .pattern_analyzer
            .analyze_command_patterns(&recent_choices);

        // Analyze context patterns
        let context_patterns = self
            .pattern_analyzer
            .analyze_context_patterns(&recent_choices);
        ...
        for (pattern_key, pattern) in command_patterns {
            self.update_pattern_with_decay(
                &mut prefs.behavior.action_patterns,
                pattern_key,
                pattern,
            );
        }
    ...
    fn update_command_statistics(
        ...
        *stats
        ...
        stats.success_rate =
            (stats.success_rate * (stats.total_uses - 1) as f32 + 0.9) / stats.total_uses as f32;
    }
    ...
    fn update_pattern_with_decay(
        ...
        if let Some(existing_pattern) = patterns.get_mut(&pattern_key) {
            ...
            existing_pattern.frequency = existing_pattern.frequency
                * (1.0 - self.learning_config.confidence_decay_rate)
    ...
    fn adjust_learning_weights(
        ...
        if adjustment < 0.0 {
            prefs.behavior.adaptation_weights.complexity_weight =
                (prefs.behavior.adaptation_weights.complexity_weight - adjustment * 0.5)
                    .clamp(0.1, 0.5);
        }
    }
    ...
    fn clean_old_patterns(&self, prefs: &mut UserPreferences) {
        ...
        prefs.behavior.action_patterns.retain(|_, pattern| {
            pattern.last_seen > cutoff_date
                && pattern.frequency > self.learning_config.min_pattern_frequency
        });
    }
...
impl PatternAnalyzer {
    ...
    pub fn analyze_command_patterns(
        &self,
        choices: &[InterfaceChoice],
    ) -> HashMap<String, ActionPattern> {
        self.pattern_detectors
            .iter()
    ...
    pub fn analyze_context_patterns(
        &self,
        choices: &[InterfaceChoice],
    ) -> HashMap<String, ActionPattern> {
        self.pattern_detectors
    ...
    fn detect_patterns(&self, choices: &[InterfaceChoice]) -> HashMap<String, ActionPattern> {
        ...
        for seq_len in self.min_sequence_length..=self.max_sequence_length.min(choices.len()) {
            for window in choices.windows(seq_len) {
                ...
                let consistency = interfaces
                    .iter()
                    .filter(|&i| i == &most_common_interface)
                    .count() as f32
                    / interfaces.len() as f32;
...
impl PatternDetector for TimeBasedPatternDetector {
    fn detect_patterns(&self, choices: &[InterfaceChoice]) -> HashMap<String, ActionPattern> {
        ...
        for (time_window, window_choices) in time_groups {
            ...
            if consistency >= 0.6 {
                // 60% consistency for time patterns
                ...
                let pattern = ActionPattern {
                    ...
                    context_factors: vec![ContextFactor::TimeOfDay(
                        time_window * self.time_window_hours,
                    )],
