...
/// has joined.
///
/// The shared-library backend returns its loaded `libloading::Library` here: the
/// operator's outputs are Arrow arrays whose FFI `release` callbacks live inside
/// the `.so`, so unloading it while the loop still holds in-flight arrays leaves
/// those callbacks dangling and SIGSEGVs when the arrays are freed. [`crate::main`]
/// binds the guard for the whole run and drops it last.
///
