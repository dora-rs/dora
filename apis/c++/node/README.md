# Adora Node API for C++

Adora supports nodes written in C++ through this API crate.
...
- Build the `adora-node-api-cxx` package:
  ```bash
  cargo build --package adora-node-api-cxx
  ```
  - This will result in `adora-node-api.h` and `adora-node-api.cc` files in the `target/cxxbridge/adora-node-api-cxx` directory.
- Include the `adora-node-api.h` header file in your source file.
- Add the `adora-node-api.cc` file to your compile and link steps.

### Build with ROS2 Bridge

Adora features an experimental ROS2 Bridge that enables adora nodes to publish and subscribe to ROS2 topics.
To enable the bridge, use these steps:

- Clone the `adora` repository (see above).
...
- Build the `adora-node-api-cxx` package **with the `ros2-bridge` feature enabled**:
  ```bash
  cargo build --package adora-node-api-cxx --features ros2-bridge
  ```
  - In addition to the `adora-node-api.h` and `adora-node-api.cc` files, this will place a `adora-ros2-bindings.h` and a `adora-ros2-bindings.cc` file in the `target/cxxbridge/adora-node-api-cxx` directory.
- Include both the `adora-node-api.h` and the `adora-ros2-bindings.h` header files in your source file.
- Add the `adora-node-api.cc` and `adora-ros2-bindings.cc` files to your compile and link steps.

## Usage

The `adora-node-api.h` header provides various functions to interact with Adora.

### Init Adora Node

All nodes need to register themselves with Adora at startup.
To do that, call the `init_adora_node()` function.
The function returns a `AdoraNode` instance, which gives access to adora events and enables sending Adora outputs.
...
The `adora_node.events` field is a stream of incoming events.
To wait for the next incoming event, call `adora_node.events->next()`:

```c++
auto event = adora_node.events->next();
```

The `next` function returns an opaque `AdoraEvent` type, which cannot be inspected from C++ directly.
Instead, use the following functions to read and destructure the event:

- `event_type(event)` returns a `AdoraEventType`, which describes the kind of event. For example, an event could be an input or a stop instruction.
  - when receiving a `AdoraEventType::AllInputsClosed`, the node should exit and not call `next` anymore
- Events of type `AdoraEventType::Input` can be downcasted using `event_as_input`:
  ```c++
  auto input = event_as_input(std::move(event));
  ```
  The function returns a `AdoraInput` instance, which has an `id` and `data` field.
...
Nodes can send outputs using the `send_output` output function and the `adora_node.send_output` field.
...
auto result = send_output(adora_node.send_output, "output_id", out_slice);
...
The `adora-ros2-bindings.h` contains function and struct definitions that allow interacting with ROS2 nodes.
...
The Adora ROS2 bridge automatically creates `create_topic_<TYPE>` functions for all messages types found in the sourced ROS2 environment.
...
You can create such a event stream from Adora's event stream using the `adora_events_into_combined` function:

```c++
auto event_stream = adora_events_into_combined(std::move(adora_node.events));
```

Alternatively, if you don't want to use Adora, you can also create an empty event stream:

```c++
auto event_stream = empty_combined_events();
```

**Note:** You should only use `empty_combined_events` if you're running your executable independent of Adora.
Ignoring the events from the `adora_node.events` channel can result in unintended behavior.

#### Receiving Messages from Combined Event Stream

The merged event stream will receive all incoming events of the node, including Adora events and ROS2 messages.
...
This returns a `event` instance of type `CombinedEvent`, which can be downcasted to Adora events or ROS2 messages.
To handle an event, you should check it's type and then downcast it:

- To check for a Adora event, you can use the `is_adora()` function. If it returns `true`, you can downcast the combined event to a Adora event using the `downcast_adora` function.
...
if (event.is_adora())
{
    ...
    // handle adora_event as described above
...
The Adora ROS2 bridge exposes these constants in the generated bindings as functions.
...
The Adora ROS2 bridge automatically creates `create_client_<TYPE>` functions for all service types found in the sourced ROS2 environment.
