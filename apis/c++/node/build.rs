...
fn main() {
    ...
    let target_dir = if let Ok(target_path) = std::env::var("ADORA_NODE_API_CXX_INSTALL") {
        PathBuf::from_str(&target_path).expect("failed to parse ADORA_NODE_API_CXX_INSTALL path")
    } else {
        target_dir.join("install")
    };
    println!("cargo:rerun-if-env-changed=ADORA_NODE_API_CXX_INSTALL");
...
mod ros2 {
    ...
    pub fn generate() -> Vec<PathBuf> {
        ...
        let generated = adora_ros2_bridge_msg_gen::generate(paths.as_slice(), &out_dir, true);
