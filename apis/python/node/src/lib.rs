...
impl Node {
    ...
    pub fn register_memory_pool(
        ...
        // Pool slot assignment (round-robin)
        let pool_slot_idx = {
        ...
        if buffer_id.starts_with("pool_") {
            ...
            if parts.len() >= 3 {
                if let Ok(slot) = parts[1].parse::<usize>() {
                    ...
                    let pool_slot = { PINNED_POOL.lock().unwrap()[slot].take() };
                    ...
                    if !shmem_ptr.is_null() {
                        ...
                        if magic == HEADER_MAGIC {
                            ...
                            if let Some(slot_data) = store_back {
                                PINNED_POOL.lock().unwrap()[slot] = Some(slot_data);
                            }
...
impl Node {
    ...
    fn try_doradma_read(
        ...
        let effective_as_cuda = pinned_type.as_deref() != Some("cpu");
