...
//! This document describes how to write [integration tests] for Adora nodes.
//!
//! [integration tests]: https://en.wikipedia.org/wiki/Integration_testing
//!
//! # Usage
//!
//! There are currently three ways to run integration tests for Adora nodes:
//!
...
//! - using [`AdoraNode::init_testing`](crate::AdoraNode::init_testing) tests independent
//!   of the `main` function
//!
//!
//! ## Using `setup_integration_testing`
//!
//! The most straightforward way to write integration tests for a Adora node is to call the
...
//! [`AdoraNode::init_from_env`](crate::AdoraNode::init_from_env) function for initialization.
//!
...
//!    let outputs = adora_node_api::integration_testing::TestingOutput::ToChannel(tx);
//!
...
//!    };
//!
//!     // setup the integration testing environment -> this will make `AdoraNode::init_from_env``
...
//!
//! Adora also supports testing nodes after compilation. This is the most comprehensive way to ensure
//! that a node behaves as expected, as the testing will be done on the exact same executable as
//! used in the dataflow.
//!
//! To enable this, the
//! [`AdoraNode::init_from_env`](crate::AdoraNode::init_from_env) function provides
//! built-in support for integration testing through environment variables.
//!
//! To start an integration test, run a node executable directly (i.e. don't go through the adora
//! cli) with the following environment variables set:
//!
//! - `ADORA_TEST_WITH_INPUTS`: Path to a JSON file that contains the inputs that should be sent
//!   to the node.
//!   - The file format is defined through the [`IntegrationTestInput`] struct (encoded as JSON).
//! - `ADORA_TEST_WRITE_OUTPUTS_TO` (optional): Path at which the output
...
//! - `ADORA_TEST_NO_OUTPUT_TIME_OFFSET` (optional): If set to any value, the output JSONL file
...
//!
//! In integration test mode, the node will not connect to any Adora daemon or other nodes. Instead,
...
//!
//! ```bash
//! > ADORA_TEST_WITH_INPUTS=path/to/inputs.json ADORA_TEST_NO_OUTPUT_TIME_OFFSET=1 cargo r -p rust-dataflow-example-status-node
//! ```
//!
//! ## Using `AdoraNode::init_testing`
//!
...
//! `main` function. To make this easier, Adora provides a
//! [`AdoraNode::init_testing`](crate::AdoraNode::init_testing) initialization function to initialize
...
//! `AdoraNode::init_from_env`, but it does not modify any global state or thread-local state.
...
//!     );
//!
//!     let outputs = adora_node_api::integration_testing::TestingOutput::ToFile(
...
//! by recording actual dataflow runs. This can be done by running a Adora dataflow with the
//! **`ADORA_WRITE_EVENTS_TO`** environment variable set to a folder path. This will instruct all
...
//! `ADORA_TEST_WITH_INPUTS` environment variable. Thus, the generated files can be directly used
...
pub use adora_message::integration_testing_format::{self, IntegrationTestInput};
...
/// [`AdoraNode::init_from_env`](crate::AdoraNode::init_from_env) to initialize
/// a testing node with the given input, output, and options.
///
/// ## Implementation Details
///
/// This function sets up thread-local state that is read by the `AdoraNode::init_from_env` function.
/// Thus, it only affects the current thread.
///
/// Calls to `AdoraNode::init_from_env` will consume the testing environment set up by this function
/// and reset the thread-local state afterwards. Thus, subsequent calls to `AdoraNode::init_from_env`
...
///   - This field is omitted if the `ADORA_TEST_NO_OUTPUT_TIME_OFFSET` environment is set.
///
///
