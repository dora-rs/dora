...
impl DoraNode {
    ...
    fn init_with_options(
        ...
        let event_stream = EventStream::init(
            ...
            clock.clone(),
            write_events_to,
            None, // zenoh session not used for node data plane
        )
    ...
    pub fn send_output_sample(
        ...
        // Always send data through the daemon control channel for reliable local
        // delivery. The daemon forwards it to local receivers via the subscribe
        // channel. Zenoh pub/sub is not used for the local data plane because
        // separate per-node zenoh sessions may not discover each other in time.
    ...
    ///
    /// Returns a 128-byte-aligned buffer suitable for Arrow data.
...
pub struct DataSample {
    data: AVec<u8, ConstAlign<128>>,
}

impl DataSample {
    fn finalize(self) -> DataMessage {
        DataMessage::Vec(self.data)
    }
}
