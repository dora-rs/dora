...
impl EventStream {
    ...
    fn correlation_expired(&mut self, needle: &str, timeout: Option<Duration>) -> bool {
        ...
        let deadline = match self.correlation_deadlines.get(needle) {
            ...
            None => {
                // Same clamp as the blocking path, so a poll asking to
                // wait `Duration::MAX` gets no practical deadline instead
                // of one that overflowed. It reads its own `now`, and the
                // check below reads the clock again — that ordering is
                // what lets a zero timeout expire on this very poll.
                let deadline = representable_wait_deadline(timeout);
...
// Non-exhaustive because we might add additional variants, e.g. an
// `AllOf` that waits for every listed server rather than the first.
#[non_exhaustive]
...
mod tests {
    ...
    /// not one that has already passed (see `representable_wait_deadline`).
    #[test]
