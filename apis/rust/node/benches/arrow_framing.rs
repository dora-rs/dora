//! Encode/decode benchmarks comparing Dora's hand-rolled raw Arrow framing
//! against Arrow IPC framing.
//!
//! Three decode strategies are measured:
//!   * `raw`                        — `buffer_into_arrow_array` (current default)
//!   * `ipc_streamreader_copy`      — `decode_arrow_ipc` (`StreamReader`, copies)
//!   * `ipc_streamdecoder_zerocopy` — `decode_arrow_ipc_zero_copy`
//!     (`StreamDecoder`, aliases an aligned input buffer)
//!
    ...
    for &n in &element_counts {
        ...
        enc.bench_with_input(BenchmarkId::new("raw", payload_bytes), &data, |b, data| {
            b.iter(|| {
                ...
                let info = copy_array_into_sample(&mut sample, data);
                black_box((sample, info));
            })
        });
        enc.bench_with_input(BenchmarkId::new("ipc", payload_bytes), &data, |b, data| {
            b.iter(|| black_box(encode_arrow_ipc(data).unwrap()))
        });
        enc.finish();

        // -------- decode (pre-encode the inputs) --------
        let total = required_data_size(&data);
        let mut sample = vec![0u8; total];
