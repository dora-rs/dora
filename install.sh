...
# Check for existing adora installations
check_existing_installations() {
  existing=""

  # Find all adora binaries in PATH
  if command -v adora > /dev/null 2>&1; then
    # Get all locations of adora
    adora_locations=$(command -v -a adora 2>/dev/null || which -a adora 2>/dev/null || true)

    for adora_path in $adora_locations; do
      [ -z "$adora_path" ] && continue
      ...
      # Skip nix symlinks
      if [ -L "$adora_path" ]; then
        link_target=$(readlink "$adora_path" 2>/dev/null || true)
        case "$link_target" in
          /nix/*) continue ;;
        esac
      fi

      existing="$existing $adora_path"
    done
  fi

  # Check common installation locations
  for check_path in \
    "$HOME/.cargo/bin/adora" \
    "/usr/local/bin/adora" \
    "/usr/bin/adora" \
    "$HOME/.local/bin/adora"
  do
  ...
  # Check for pip-installed adora-rs-cli
  if command -v pip > /dev/null 2>&1; then
    if pip show adora-rs-cli > /dev/null 2>&1; then
      existing="$existing pip:adora-rs-cli"
    fi
  fi
...
if [ -n "$existing_installations" ]; then
  echo ""
  echo "WARNING: Found existing adora installation(s):"
  for inst in $existing_installations; do
    if [ "$inst" = "pip:adora-rs-cli" ]; then
      echo "  - adora-rs-cli (installed via pip)"
    else
  ...
  if [ -z "${GITHUB_ACTIONS-}" ] && [ -t 0 ]; then
    ...
    case "$response" in
      [yY]|[yY][eE][sS])
        for inst in $existing_installations; do
          if [ "$inst" = "pip:adora-rs-cli" ]; then
            echo "Removing pip-installed adora-rs-cli..."
            pip uninstall -y adora-rs-cli || echo "Failed to uninstall pip package"
          else
            echo "Removing $inst..."
...
# Check adora-rs pip package version compatibility
check_pip_adora_rs() {
  if ! command -v pip > /dev/null 2>&1; then
    return
  fi

  # Check if adora-rs is installed
  pip_version=$(pip show adora-rs 2>/dev/null | grep "^Version:" | cut -d' ' -f2 || true)
  ...
  echo ""
  echo "Found adora-rs Python package version $pip_version"

  if [ "$pip_major_minor" = "$tag_major_minor" ]; then
    echo "adora-rs pip package ($pip_version) matches CLI version ($tag_version) - OK"
  else
    echo "WARNING: adora-rs pip package ($pip_version) does not match CLI version ($tag_version)"
    echo "Mismatched versions may cause compatibility issues."
    echo ""

    # Only prompt if running interactively (not in CI)
    if [ -z "${GITHUB_ACTIONS-}" ] && [ -t 0 ]; then
      printf "Do you want to upgrade adora-rs pip package to %s? [Y/n] " "$tag_version"
      read -r response
      case "$response" in
        [nN]|[nN][oO])
          echo "Skipping adora-rs pip package upgrade."
          ;;
        *)
          echo "Upgrading adora-rs pip package to $tag_version..."
          if pip install "adora-rs==$tag_version" 2>/dev/null; then
            echo "adora-rs pip package upgraded to $tag_version"
          else
            echo "Failed to upgrade adora-rs. You may need to run: pip install adora-rs==$tag_version"
          fi
          ;;
      esac
    else
      echo "Running in non-interactive mode."
      echo "To sync versions, run: pip install adora-rs==$tag_version"
    fi
  fi
}
...
# Check Cargo.toml adora-node-api version compatibility (recursive for workspaces)
check_cargo_toml_version() {
  ...
  find . -name "Cargo.toml" -not -path "*/target/*" 2>/dev/null | while IFS= read -r cargo_toml; do
    ...
    # Check for git-based dependency with tag: adora-node-api = { git = "...", tag = "v0.3.12" }
    cargo_git_tag=$(grep -E 'adora-node-api *=' "$cargo_toml" 2>/dev/null | head -1 | sed -n 's/.*tag *= *"\([^"]*\)".*/\1/p' || true)
    ...
    if [ "$cargo_major_minor" != "$tag_major_minor" ]; then
      ...
      echo "  - $cargo_toml (adora-node-api $cargo_display)" >> "${td}/mismatched_display"
    else
  ...
  if [ -n "$matched_version" ] && [ -z "$mismatched_files" ]; then
    echo ""
    echo "Found Cargo.toml with adora-node-api version $matched_version - matches CLI version ($tag_version) - OK"
  elif [ -n "$mismatched_files" ]; then
    echo ""
    echo "WARNING: Found Cargo.toml file(s) with adora-node-api version that does not match CLI version ($tag_version):"
    ...
    # Only prompt if running interactively (not in CI)
    if [ -z "${GITHUB_ACTIONS-}" ] && [ -t 0 ]; then
      printf "Do you want to update these Cargo.toml files to use adora-node-api %s? [y/N] " "$tag_version"
      read -r response
      case "$response" in
        [yY]|[yY][eE][sS])
          for cargo_toml in $mismatched_list; do
            ...
            if grep -qE 'adora-node-api *= *\{[^}]*tag *=' "$cargo_toml" 2>/dev/null; then
              sed -i.bak -E "s/(adora-node-api *= *\{[^}]*tag *= *\")[^\"]*(\")/\1$tag\2/" "$cargo_toml" && rm -f "$cargo_toml.bak"
            # Update version in table format: { version = "X.Y.Z" }
            elif grep -qE 'adora-node-api *= *\{[^}]*version *=' "$cargo_toml" 2>/dev/null; then
              sed -i.bak -E "s/(adora-node-api *= *\{[^}]*version *= *\")[^\"]*(\")/\1$tag_version\2/" "$cargo_toml" && rm -f "$cargo_toml.bak"
            # Update version in simple format: "X.Y.Z"
            elif grep -qE 'adora-node-api *= *"' "$cargo_toml" 2>/dev/null; then
              sed -i.bak -E "s/(adora-node-api *= *\")[^\"]*(\")/\1$tag_version\2/" "$cargo_toml" && rm -f "$cargo_toml.bak"
            fi
          done
          echo "Cargo.toml files updated to adora-node-api $tag_version"
          ;;
        *)
...
# Check pyproject.toml adora-rs version compatibility
check_pyproject_toml_version() {
  ...
  find . -name "pyproject.toml" -not -path "*/.*" 2>/dev/null | while IFS= read -r pyproject; do
    # Extract adora-rs version from pyproject.toml
    # Matches patterns like: adora-rs==0.4.0, adora-rs>=0.4.0, adora-rs = "^0.4.0", "adora-rs==0.4.0"
    pyproject_version=$(grep -E 'adora-rs' "$pyproject" 2>/dev/null | head -1 | sed -n 's/.*adora-rs[^0-9]*\([0-9][0-9]*\.[0-9][0-9]*\.[0-9][0-9]*\).*/\1/p' || true)
    ...
    if [ "$pyproject_major_minor" != "$tag_major_minor" ]; then
      ...
      echo "  - $pyproject (adora-rs $pyproject_version)" >> "${td}/mismatched_pyproject_display"
    else
  ...
  if [ -n "$matched_pyproject_version" ] && [ -z "$mismatched_pyproject_files" ]; then
    echo ""
    echo "Found pyproject.toml with adora-rs version $matched_pyproject_version - matches CLI version ($tag_version) - OK"
  elif [ -n "$mismatched_pyproject_files" ]; then
    echo ""
    echo "WARNING: Found pyproject.toml file(s) with adora-rs version that does not match CLI version ($tag_version):"
    ...
    # Only prompt if running interactively (not in CI)
    if [ -z "${GITHUB_ACTIONS-}" ] && [ -t 0 ]; then
      printf "Do you want to update these pyproject.toml files to use adora-rs %s? [y/N] " "$tag_version"
      read -r response
      case "$response" in
        [yY]|[yY][eE][sS])
          for pyproject in $mismatched_pyproject_list; do
            echo "Updating $pyproject..."
            # Update version patterns like adora-rs==X.Y.Z or adora-rs>=X.Y.Z
            sed -i.bak -E "s/(adora-rs[><=]*)[0-9]+\.[0-9]+\.[0-9]+/\1$tag_version/g" "$pyproject" && rm -f "$pyproject.bak"
          done
          echo "pyproject.toml files updated to adora-rs $tag_version"
          ;;
        *)
...
# Check requirements.txt adora-rs version compatibility
check_requirements_txt_version() {
  ...
  find . -name "requirements*.txt" -not -path "*/.*" 2>/dev/null | while IFS= read -r reqfile; do
    # Extract adora-rs version from requirements.txt
    # Matches patterns like: adora-rs==0.4.0, adora-rs>=0.4.0, adora-rs~=0.4.0
    req_version=$(grep -E '^adora-rs' "$reqfile" 2>/dev/null | head -1 | sed -n 's/.*adora-rs[^0-9]*\([0-9][0-9]*\.[0-9][0-9]*\.[0-9][0-9]*\).*/\1/p' || true)
    ...
    if [ "$req_major_minor" != "$tag_major_minor" ]; then
      ...
      echo "  - $reqfile (adora-rs $req_version)" >> "${td}/mismatched_req_display"
    else
  ...
  if [ -n "$matched_req_version" ] && [ -z "$mismatched_req_files" ]; then
    echo ""
    echo "Found requirements.txt with adora-rs version $matched_req_version - matches CLI version ($tag_version) - OK"
  elif [ -n "$mismatched_req_files" ]; then
    echo ""
    echo "WARNING: Found requirements.txt file(s) with adora-rs version that does not match CLI version ($tag_version):"
    ...
    # Only prompt if running interactively (not in CI)
    if [ -z "${GITHUB_ACTIONS-}" ] && [ -t 0 ]; then
      printf "Do you want to update these requirements.txt files to use adora-rs %s? [y/N] " "$tag_version"
      read -r response
      case "$response" in
        [yY]|[yY][eE][sS])
          for reqfile in $mismatched_req_list; do
            ...
            # Update version patterns like adora-rs==X.Y.Z or adora-rs>=X.Y.Z
            sed -i.bak -E "s/^(adora-rs[><=~]*)[0-9]+\.[0-9]+\.[0-9]+/\1$tag_version/" "$reqfile" && rm -f "$reqfile.bak"
          done
          echo "requirements.txt files updated to adora-rs $tag_version"
          ;;
        *)
