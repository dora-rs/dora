...
# Exercise every example dataflow end-to-end (~15-20 min, orthogonal to ladder)
make qa-examples
...
| `make qa-examples` | `scripts/smoke-all.sh` -- every example dataflow end-to-end | ~15-20 min | When you want actual dataflows exercised. Orthogonal to ladder -- qa-fast/full/deep all `--exclude dora-examples`. Pass `ARGS="--rust-only"` etc. |
