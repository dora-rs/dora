...
*Phase 5b (C++ action server) — landed-pending.* New cxx codegen `cxx_action_server_creation_functions` in `msg-gen/src/types/action.rs`, mirroring the daemon's `run_action_server` FSM: a dedicated-thread goal pump receives goals, auto-accepts + starts executing them (rejecting once `MAX_CONCURRENT_GOALS` are in flight, so the handle map can't leak), stores each `ExecutingGoalHandle` (keyed by goal-id `uuid`) and forwards `(goal_id, goal)` to the merged stream; the C++ side does `matches` → `downcast` (→ a `GoalEvent` exposing `get_goal_id()` + `get_goal()`), then `publish_feedback(goal_id, feedback)` and `send_result(goal_id, status, result)`, which look the handle back up. The existing shared `ActionGoalId` cxx type carries the goal id; `ExecutingGoalHandle` is parameterized by the **goal-message** type (not the action marker) so it is `Clone`. Example: `examples/ros2-bridge/c++/action-server/` (`Fibonacci` server) paired with the existing C++ action client (both ros2-client) in one self-contained dataflow. **Verification posture (same as the Rust action server, Phase 4):** the codegen + C++ API compile across all action types, and on the macOS arm64 harness the cxx client discovers the cxx server and the goal is received (`received goal (order 10)`); the deferred `get_result` round-trip does not complete on arm64 (documented #1972-family limitation), so the **x86 nightly C++ job is the oracle** (added here). A real rmw client cannot discover the server (ros2-client#4), which is why the example's peer is another dora node. Not added to `scripts/ros2dev.sh` (the get_result stall would hang it on arm64).

With Phase 5b, **every #1170 sub-item is covered**: Rust/Python/C++ each have topic pub/sub + service client/server + action client/server, plus the Python parameter service.
