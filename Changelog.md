...
- **`dora clean` subcommand**: removes finished and failed dataflows from the coordinator's state without restarting the coordinator. Drains `dataflow_results` and their archived descriptors; deliberately does NOT touch `finished_builds` (which would break concurrent `dora build` calls with "unknown build id" errors). Useful for dev workflows that accumulate dataflow history and long-running coordinators that want to trim state. Cleaned dataflows are no longer queryable via `dora logs <uuid>`. Rescue of [#1366](https://github.com/dora-rs/dora/pull/1366) (reimplemented against the current coordinator since the original PR targeted a control plane that was rewritten).
