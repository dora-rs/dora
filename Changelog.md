...
- **Module inner-node source paths resolve relative to the module file.** A module's inner-node `path:` already did; its operators' `shared-library:`, `python:`, and `wasm:` now do too. Previously an operator source inside a module was resolved against the *dataflow* directory, so a module in `modules/` declaring `shared-library: proc` produced the bare dlopen name `libproc.so` — no slash, so `dlopen` searched the system paths and never the module directory. Two behaviour changes follow: a source that resolves outside the dataflow directory (e.g. `python: ../shared/op.py`) is now rejected at expansion time rather than silently resolved against the wrong root, and `build:` commands still run with the dataflow directory as their working directory, so a built artifact must be referenced from where the build put it. Also fixes `path: dynamic` / `path: shell` inside a module in a subdirectory, which were rewritten to `modules/nested/dynamic` and no longer matched the daemon's exact-string check.
