...
- **Non-blocking correlated receives** ([#3046](https://github.com/dora-rs/dora/issues/3046)). `try_recv_service_response` / `try_recv_action_result` (and their `_from` variants) return `Ok(None)` when the reply has not arrived, so a single-threaded node can keep several requests in flight without blocking on any one of them. The deadline is owned by the framework: the first poll carrying one registers it, and a later poll past it reports `Timeout` once per deadline, so callers do not sweep deadlines themselves. `cancel_correlation` releases a request that will never be polled again. Exposed to C++ with a `NotReady` pattern status, distinct from `Timeout`. `examples/c++-service-action/` gains a polling client.
