...
- **Nodes no longer outlive a SIGKILLed `dora run`** (Unix): the in-process daemon now marks every node it spawns with the run process's pid (`DORA_LOCAL_RUN_PID`), and the node API watches that pid from all init paths (custom nodes, the operator runtime, and the C/C++/Python bindings). When the `dora run` process dies without teardown — `kill -9`, a supervisor hard-timeout, an OOM kill — each node sends itself a cooperative SIGTERM, waits a 5 s grace for handlers, then hard-exits. Previously such nodes were orphaned indefinitely (`ppid 1`, own process group, unreachable by any group kill), because nodes are spawned as process-group leaders and a node that is not polling its event stream never observes the daemon-socket EOF. Scoped to `dora run` only: the coordinator-attached spawn path injects nothing, so nodes there still survive daemon restarts (#2029). The variable is control-plane-denylisted — a descriptor cannot forge it and a stale shell export cannot leak it. See [#2856](https://github.com/dora-rs/dora/issues/2856).
