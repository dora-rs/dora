...
- `make qa-nightly` (qa-deep + proptest@1000 + miri + example-smoke suite) — ~60-90 minutes. Tier 2 equivalent locally **plus CI-nightly smoke parity**: the final step runs `cargo test -p dora-examples --test example-smoke -- --test-threads=1`, which is what the GHA nightly's smoke-suite / log-sinks / service-action / streaming / record-replay jobs exercise. A green local `qa-nightly` maps to a green CI nightly on those jobs. Skips miri if `cargo +nightly miri` isn't installed. **Does not** include full-repo mutation testing (split into `qa-mutation-audit`, ~10-18 hrs) nor the specialized CI-only jobs (cluster-smoke, cpu-affinity-smoke, redb-backend-smoke, daemon-reconnect-smoke, state-reconstruction-smoke, topic-and-top-smoke) which have extra env setup.
