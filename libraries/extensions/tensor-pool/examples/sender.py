#!/usr/bin/env python
"""Send tensors through the tensor-pool example dataflow."""
...
SCENARIO = os.getenv("tensor_pool_scenario", "throughput")
...
for i in range(MESSAGE_COUNT):
    ...
    if i == 0:
        ...
        tensor_pool_id = node.register_tensor_pool(tensor_info, RECEIVER_DEVICE)
        node.send_output("data", tensor_pool_id, metadata)
    else:
        tensor_info = get_tensor_info(torch_tensor)
        if SCENARIO == "write_after_free" and i == 1:
            node.free_tensor_pool(tensor_pool_id)
        node.write_tensor_pool(tensor_pool_id, tensor_info)
