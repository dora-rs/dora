...
fn convert_twist_to_ros(py: Python, array: &ArrayRef) -> PyResult<PyObject> {
    ...
    if let Some(linear_col) = struct_array.column_by_name("linear") {
        let linear_struct = linear_col.as_any().downcast_ref::<StructArray>()
            .ok_or_else(|| PyErr::new::<pyo3::exceptions::PyValueError, _>("Expected linear struct"))?;
    ...
    if let Some(angular_col) = struct_array.column_by_name("angular") {
        let angular_struct = angular_col.as_any().downcast_ref::<StructArray>()
            .ok_or_else(|| PyErr::new::<pyo3::exceptions::PyValueError, _>("Expected angular struct"))?;
...
fn convert_header_to_ros(py: Python, array: &ArrayRef) -> PyResult<PyObject> {
    ...
    if let Some(stamp_col) = struct_array.column_by_name("stamp") {
        let stamp_struct = stamp_col.as_any().downcast_ref::<StructArray>()
            .ok_or_else(|| PyErr::new::<pyo3::exceptions::PyValueError, _>("Expected stamp struct"))?;
    ...
    if let Some(frame_id) = struct_array.column_by_name("frame_id") {
        let frame_id_str = frame_id.as_any().downcast_ref::<arrow::array::StringArray>()
            .ok_or_else(|| PyErr::new::<pyo3::exceptions::PyValueError, _>("Expected string array"))?
            .value(0);
...
fn convert_pose_to_ros(py: Python, array: &ArrayRef) -> PyResult<PyObject> {
    ...
    if let Some(position_col) = struct_array.column_by_name("position") {
        let position_struct = position_col.as_any().downcast_ref::<StructArray>()
            .ok_or_else(|| PyErr::new::<pyo3::exceptions::PyValueError, _>("Expected position struct"))?;
    ...
    if let Some(orientation_col) = struct_array.column_by_name("orientation") {
        let orientation_struct = orientation_col.as_any().downcast_ref::<StructArray>()
            .ok_or_else(|| PyErr::new::<pyo3::exceptions::PyValueError, _>("Expected orientation struct"))?;
...
fn convert_transform_stamped_to_ros(py: Python, array: &ArrayRef) -> PyResult<PyObject> {
    ...
    if let Some(child_frame_id) = struct_array.column_by_name("child_frame_id") {
        let child_frame_id_str = child_frame_id.as_any().downcast_ref::<arrow::array::StringArray>()
            .ok_or_else(|| PyErr::new::<pyo3::exceptions::PyValueError, _>("Expected string array"))?
            .value(0);
...
fn convert_image_to_ros(py: Python, array: &ArrayRef) -> PyResult<PyObject> {
    ...
    if let Some(encoding) = struct_array.column_by_name("encoding") {
        let encoding_str = encoding.as_any().downcast_ref::<arrow::array::StringArray>()
            .ok_or_else(|| PyErr::new::<pyo3::exceptions::PyValueError, _>("Expected string array"))?
            .value(0);
    ...
    if let Some(data) = struct_array.column_by_name("data") {
        ...
        let binary_array = data.as_any().downcast_ref::<arrow::array::BinaryArray>()
            .ok_or_else(|| PyErr::new::<pyo3::exceptions::PyValueError, _>("Expected binary array"))?;
...
fn convert_compressed_image_to_ros(py: Python, array: &ArrayRef) -> PyResult<PyObject> {
    ...
    if let Some(format) = struct_array.column_by_name("format") {
        let format_str = format.as_any().downcast_ref::<arrow::array::StringArray>()
            .ok_or_else(|| PyErr::new::<pyo3::exceptions::PyValueError, _>("Expected string array"))?
            .value(0);
...
fn convert_laserscan_to_ros(py: Python, array: &ArrayRef) -> PyResult<PyObject> {
    ...
    if let Some(ranges) = struct_array.column_by_name("ranges") {
        ...
        let list_array = ranges.as_any().downcast_ref::<arrow::array::ListArray>()
            .ok_or_else(|| PyErr::new::<pyo3::exceptions::PyValueError, _>("Expected list array"))?;
    ...
    for (i, field) in struct_array.fields().iter().enumerate() {
        ...
        match column.data_type() {
            arrow::datatypes::DataType::Float64 => {
                if let Some(primitive) = column.as_any().downcast_ref::<arrow::array::Float64Array>() {
