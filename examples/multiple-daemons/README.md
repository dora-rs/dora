...
Adora supports splitting a dataflow onto multiple daemon instances, which typically run on different machines.
...
- Build all nodes of the dataflow by running `adora build dataflow.yml`.
- Start a coordinator by running `adora coordinator`.
  - If you want more log output, run `RUST_LOG=debug adora coordinator` instead.
- In a new terminal, start the first daemon instance by running `adora daemon --machine-id A`.
...
- In a third terminal, start the second daemon instance by running `adora daemon --machine-id B --local-listen-port 53292`.
...
- Start the dataflow through `adora start dataflow.yml`

# Usage Across Multiple Machines

1. Start the adora coordinator on some machine that is reachable by IP by all other machines
    ```bash
    adora coordinator
    ```
    - If you plan to run adora in the cloud, the machine that runs the `adora coordinator` needs to have a public IP.
    - Set a `RUST_LOG` environment variable to get more output on the command line. For example, `RUST_LOG=debug adora coordinator` reports when daemons connect and when dataflow start commands are received.
2. Start adora daemon instances on all machines that should run parts of the dataflow.
    - If all daemons and the coordinator are **in the same network** (e.g. same WiFi), use the following command:
      ```bash
      RUST_LOG=debug adora daemon --coordinator-addr <IP> --machine-id <MACHINE_ID>
      ```
    ...
    - If daemons/coordinator are in **different networks** (e.g. behind a [NAT](https://en.wikipedia.org/wiki/Network_address_translation)), you need to set up one or multiple [zenoh routers](https://zenoh.io/docs/getting-started/deployment/#zenoh-router). This requires running a [`zenohd`](https://zenoh.io/docs/getting-started/installation/) instance for every subnet. This is also possible [using a Docker container](https://zenoh.io/docs/getting-started/quick-test/).
      ...
      Once your `zenohd` instances are running, you can start the `adora daemon` instances as before, but now with a `ZENOH_CONFIG` environment variable set:
      ```bash
      ZENOH_CONFIG=<ZENOH_CONFIG_FILE_PATH> RUST_LOG=debug adora daemon --coordinator-addr <IP> --machine-id <MACHINE_ID>
      ```
...
3. In your `dataflow.yml` file, add an `_unstable_deploy` key to all nodes that should not run on the default machine:
    ...
    - id: example-node
      ...
      path: /home/user/adora/target/debug/rust-dataflow-example-node
      inputs: [...]
      outputs: [...]
    ```
...
4. Build all the nodes on their target machines, or build them locally and copy them over.
  ...
  Adora does **not** do any deployment of executables yet. You need to copy the executables/Python code yourself.
5. Use the CLI to start the dataflow:
  ```bash
  RUST_LOG=debug adora start --coordinator-addr <IP> dataflow.yml
  ```
...
Adora publishes outputs destined for remote nodes on the following zenoh topic names:

```
adora/{network_id}/{dataflow_id}/output/{node_id}/{output_id}
```

- The `{network_id}` is always `default` for now. In the future, we plan to make it configurable to allow operating multiple separate adora networks within the same WiFi.
