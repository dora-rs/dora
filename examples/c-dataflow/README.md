...
This examples shows how to create and connect adora operators and custom nodes in C.
...
- Compile the `node.c` (e.g. using `clang`) and link the staticlib
  - For example, use the following command:
    ```
    clang node.c <FLAGS> -ladora_node_api_c -L ../../target/release --output build/c_node
    ```
