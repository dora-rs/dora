...
class ActInference:
    ...
    def get_image(self, request=True):
        ...
        while not got_image:
            if request:
                self.node.send_output(output_id="request_camera", data=pa.array([]), metadata={})
            event = self.node.next(1)
            if event is not None and event["type"] == "INPUT" and event["id"] == "camera":
    ...
    def get_qpos(self, request=True):
        ...
        while not got_qpos:
            if request:
                self.node.send_output(output_id="request_joint_pos", data=pa.array([]), metadata={})
            event = self.node.next(1)
            if event is not None and event["type"] == "INPUT" and event["id"] == "joint_pos":
