        ...
        with mujoco.viewer.launch_passive(model, data) as viewer:
            ...
            for event in node:
                if event["type"] == "INPUT" and event["id"] == "cmd_vel":
                    ...
                    linear_x = clamp(
                        float(cmd_vel[0]), -max_linear_velocity, max_linear_velocity
                    )
                    angular_z = clamp(
                        float(cmd_vel[5]), -max_angular_velocity, max_angular_velocity
                    )
                    ...
                    left_velocity = (
                        linear_x - angular_z * wheel_separation / 2.0
                    ) / wheel_radius
                    right_velocity = (
                        linear_x + angular_z * wheel_separation / 2.0
                    ) / wheel_radius
